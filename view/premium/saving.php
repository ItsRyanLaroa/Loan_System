...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['distribute_income'])) {
        $message = distributeIncome();
        echo "<p>$message</p>";
    }
}
...
// Function to distribute income among premium members
function distributeIncome() {
    global $conn;

    // Get the total company income for the current year
    $year = date('Y');
    $sql = "SELECT total_income FROM company_income WHERE year = $year";
    $result = $conn->query($sql);

    if (!$result || $result->num_rows == 0) {
        return "No income data available for the current year.";
    }
    ...
    $distribution_amount = $total_income * 0.02;

    // Get the count of premium members
    $sql = "SELECT COUNT(*) as premium_count FROM premium_users WHERE is_premium = 1";
    $result = $conn->query($sql);

    if (!$result || $result->num_rows == 0) {
        return "No premium members found.";
    }
    ...
    // Calculate the amount for each premium member
    $amount_per_member = $distribution_amount / $premium_count;

    // Get all premium members
    $sql = "SELECT user_id FROM premium_users WHERE is_premium = 1";
    $result = $conn->query($sql);

    if (!$result) {
        return "Error fetching premium members: " . $conn->error;
    }
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        // Calculate the new amount
        $new_amount = $current_amount + $amount_per_member;
        
        // Generate a unique transaction ID
        $transaction_id = generateUniqueTransactionID();
        
        $transaction_type = 'Income Distribution';
        $status = 'Completed';
        $created_at = date('Y-m-d H:i:s');
        
        // Prepare the query
        $sql = "INSERT INTO savingstransactions (user_id, transaction_id, transaction_type, amount, current_amount, status, created_at) VALUES ('$user_id', '$transaction_id', '$transaction_type', '$amount_per_member', '$new_amount', '$status', '$created_at')";
        
        // Execute the query
        if (!$conn->query($sql)) {
            return "Error updating savings for user $user_id: " . $conn->error;
        }
    }

    return "Income distributed successfully among premium members.";
}
    ...
    <div class="container">
        ...
        <div class="content">
            ...
            <form method="POST" action="saving.php">
                <button type="submit" name="distribute_income">Distribute Income to Premium Members</button>
            </form>
